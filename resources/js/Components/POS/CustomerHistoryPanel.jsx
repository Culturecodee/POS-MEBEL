...
export default function CustomerHistoryPanel({
    ...
    if (loading) {
        return (
            <div className="p-6 flex items-center justify-center">
                <IconLoader2
                    size={24}
                    className="animate-spin text-[#7b563f]"
                />
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="p-6 text-center">
                <p className="text-sm text-[#8a4f35] dark:text-[#e3b79f]">{error}</p>
            </div>
        );
    }
    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden">
            {/* Header */}
            <div className="bg-gradient-to-r from-[#7b563f] to-[#5f3f2d] px-4 py-3 flex items-center justify-between">
            ...
            <div className="grid grid-cols-3 gap-px bg-slate-100 dark:bg-slate-800">
                <div className="bg-white dark:bg-slate-900 p-3 text-center">
                    <div className="flex items-center justify-center mb-1">
                        <IconReceipt size={16} className="text-[#7b563f]" />
                    </div>
                ...
                <div className="bg-white dark:bg-slate-900 p-3 text-center">
                    <div className="flex items-center justify-center mb-1">
                        <IconCoin size={16} className="text-[#a06d46]" />
                    </div>
                    <p className="text-sm font-bold text-[#a06d46] dark:text-[#e2bd8d]">
                ...
                <div className="px-4 py-3 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {frequent_products.map((product) => (
                            <span
                                key={product.id}
                                className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-[#fbf4eb] dark:bg-[#5f3f2d]/30 text-xs font-medium text-[#7b563f] dark:text-[#ead7bf]"
                            >
                                {product.title}
                                <span className="text-[#7b563f]">
                                    ×{product.total_qty}
                                </span>
                            </span>
                        ))}
    ...
    return (
        <>
            <button
                ...
                className={`p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 hover:text-[#7b563f] transition-colors ${className}`}
                title="Lihat riwayat"
            >
                <IconHistory size={16} />
            </button>
